...
class ReviewController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        if($product)
        {
            ...
            if($new_review)
            {
                return redirect('category/'.$category_slug.'/'.$prod_slug)->with('status','Cảm ơn bạn đã đánh giá');
            }
        }
        else
        {
            return redirect()->back()->with('status','Liên kết bạn theo dõi đã bị hỏng');
        }
    }
    ...
    public function update(Request $request)
    {
        ...
        if($user_review != '' )
        {
            ...
            if($review)
            {
                ...
                $review->update();
                return redirect('category/'.$review->product->category->slug.'/'.$review->product->slug)->with('status','Cập nhập đánh giá');
            }
            else
            {
                return redirect()->back()->with('status','The link  you followed was broken');
        ...
        {
            return redirect()->back()->with('status','Bạn không được để đánh giá trống');
        }
